...
__export(index_exports, {
  AnotherSmartString: () => AnotherSmartString,
  BPromise: () => BPromise,
  ...
  GroupBuilderZodlessAccumelating: () => GroupBuilderZodlessAccumelating,
  OneOf: () => OneOf,
  Optionable: () => Optionable,
  OrderedRecord: () => OrderedRecord,
  OrderedRecordBase: () => OrderedRecordBase,
  Port: () => Port,
  Result: () => Result,
  ...
  entries: () => entries,
  ifNotNone: () => ifNotNone,
  logWithoutMethods: () => logWithoutMethods,
  map: () => map2,
...
// src/data_structures/array.ts
var BetterArray = class _BetterArray {
  data;
  constructor(data) {
    this.data = data;
  }
  filter(isValueValid) {
    return new _BetterArray(this.data.filter(isValueValid));
  }
  tick(callback) {
    callback(this.data);
    return this.data;
...
// src/errors-as-values/rust-like-pattern/leftRight.ts
var LeftRight = class {
  left;
  right;
...
// src/errors-as-values/rust-like-pattern/mapable.ts
var Mapable = class {
  v;
  constructor(v) {
    this.v = v;
...
// src/errors-as-values/rust-like-pattern/unpackable/unpackable.ts
var CustomUnpackable = class {
  value;
  canBeUnpacked;
...
// src/errors-as-values/rust-like-pattern/option.ts
function Try(v, config) {
  if (v === void 0 || v === null) {
...
var Optionable = class extends CustomUnpackable {
  constructor(v) {
    super(v, (v2) => {
      return !this.is_none();
    });
...
function ifNotNone(v, callback) {
  return new Optionable(v).ifCanBeUnpacked(callback);
}

// src/data_structures/getSetClass.ts
var GetSet = class {
  v;
  onSet = (v) => {
  };
  ...
  constructor(v, onGet, onSet) {
    this.v = v;
    ifNotNone(onGet, (onGet2) => this.onGet = onGet2);
    ifNotNone(onSet, (onSet2) => this.onSet = onSet2);
  }
...
// src/data_structures/group.ts
var GroupBuilder = class _GroupBuilder {
  funcs;
  state;
  constructor(funcs, state) {
    this.funcs = funcs;
    this.state = state;
  }
  addFunc(v) {
    return new _GroupBuilder([...this.funcs, v], this.state);
  }
  build() {
    return (v) => this.funcs.forEach((func) => func(v));
  }
};
var group = new GroupBuilder([], esm_default.object({
  dockerImage: esm_default.string(),
  port: esm_default.number()
})).addFunc((state) => {
  state.dockerImage = "dockerImage";
}).addFunc((state) => {
  state.port = 1;
}).build();
var GroupBuilderZodlessAccumelating = class _GroupBuilderZodlessAccumelating {
  funcs;
  state;
  constructor(funcs, state) {
    this.funcs = funcs;
  ...
  addFunc(v) {
    return new _GroupBuilderZodlessAccumelating([...this.funcs, v], this.state);
  }
  build() {
    return (v) => this.funcs.forEach((func) => func(v));
  }
...
((OneOf2) => {
  class Instance {
    ...
    handlers = {};
    schema;
    constructor(v, handlers) {
      this.value = v;
      ifNotNone(handlers, (h2) => this.handlers = h2);
    }
    if(config) {
  ...
  OneOf2.Instance = Instance;
})(OneOf || (OneOf = {}));

// src/data_structures/RecordCompatibeArray.ts
var OrderedRecordBase = class _OrderedRecordBase extends Array {
  get getElementsType() {
  ...
  placeBefore(key, newValue) {
    return this.add(
      newValue,
      this.elements.value.findIndex((el) => el.key === key)
    );
  }
  placeAfter(key, newValue) {
    return this.add(
      newValue,
  ...
  add(v, position) {
    ...
    let newElements;
    if (position === void 0) {
      newElements = [...this.elements.value, v];
    } else {
      newElements = [
    ...
    return new _OrderedRecordBase(newElements);
  }
};
var OrderedRecord = class extends OrderedRecordBase {
  get lastElement() {
    return this.elements.value[this.elements.value.length - 1];
  }
};

// src/errors-as-values/rust-like-pattern/result.ts
...
var Result = class _Result extends CustomUnpackable {
  error;
  safeMode = true;
  constructor(value, error) {
    ...
    error.ifCanBeUnpacked((definedError) => {
      Object.entries(definedError).forEach(([key, value2]) => {
        value2.ifCanBeUnpacked((v) => {
          value2.ifCanBeUnpacked(
            (v2) => this.messageWhenYouCntUnpack = v2.message
          );
        });
      });
    });
  ...
  static Ok(v) {
    return new _Result(new Optionable2(v), new Optionable2(null));
  }
  static NotOk(e) {
    return new _Result(new Optionable2(null), new Optionable2(e));
  }
  static async transformFunctionThatThrowsIntoResult(functionThatCouldThrow) {
    try {
      return new _Result(
        new Optionable2(await functionThatCouldThrow()),
        new Optionable2(null)
      );
    } catch (err) {
      return new _Result(
        new Optionable2(null),
        new Optionable2(
          { errorThrownFromTheFunction: new Optionable2(new CustomError(err.message)) }
        )
      );
    }
  }
...
var ConcreteResult = class extends Result {
};
function objectEntries(obj) {
  return Object.entries(obj);
}
var Optionable2 = class _Optionable {
  constructor(value) {
    this.value = value;
  }
  getValue() {
  ...
  map(fn) {
    if (this.value === null) {
      return new _Optionable(null);
    }
    return new _Optionable(fn(this.value));
  }
};
...
var SimpleResult = class {
  ...
  ifOk(fn) {
    if (this.isOk()) {
      return new Optionable2(fn(this.value.data));
    }
    return new Optionable2(null);
  }
  onError(func) {
  ...
  ifError(handlers) {
    ...
    return new Optionable2(this.onError((error) => {
      const entry = objectEntries(handlers).find(([key, handler]) => key === error.type);
      return map(entry, ([key, handler]) => handler(error));
    }));
  }
...
var successResult = new SimpleResult({ ok: true, data: "John Doe" });
var errorResult = new SimpleResult({
  ok: false,
  type: "userNotFound",
...
// src/data_structures/safestring.ts
var SmartString = class {
  value;
  constructor(v) {
    this.value = v;
  }
...
var g = new SmartString("g h   ");
var a = g.getPart("g");
var h = g.getParts();
var AnotherSmartString;
((AnotherSmartString2) => {
  class V1 {
    schema;
    value;
  ...
  AnotherSmartString2.V2 = V2;
  const hh = new V2(["k"]);
})(AnotherSmartString || (AnotherSmartString = {}));

// src/errors-as-values/mapThatIsLikeInRust.ts
function map2(v, func) {
  return func(v);
}
function tap(v, func) {
  func(v);
...
function logWithoutMethods(h2) {
  console.log(prettifyJson(JSON.stringify(h2)));
}
...
// src/errors-as-values/baseContextSafeTypet.ts
var ContextSafeType = class {
  v;
  constructor(v) {
...
// src/types/networking/port.ts
var Port = class extends ContextSafeType {
  customValidator(v) {
    if (v < 0 || v > 65535)
      return false;
    return true;
...
// src/types/networking/urls/websocket.ts
var WebsocketUrl = class _WebsocketUrl extends ContextSafeType {
  customValidator(v) {
    if (!v.includes("ws://")) {
      return false;
    }
  ...
  static unsafe = {
    withLocalhost: (port) => {
      return new _WebsocketUrl(`ws://localhost:${port.value}`);
    }
  };
};

// src/types/promises/boolean.ts
