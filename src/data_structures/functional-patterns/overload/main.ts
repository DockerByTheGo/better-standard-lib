...
import { ifTrue } from "@better-standard-internal/type-level-functions/boolean/isBoolean";
...
class OverloadBuilderWithCustomResolver<T extends ((v: unknown) => unknown)[], isOverloadChooserDefined extends boolean = false> {
    ...
    addResolver(resolver: (overloads: T, value: Parameters<T[number]>[0]) => ReturnType<T[number]>): ifTrue<
        isOverloadChooserDefined,
